    ...
    <div class="col-lg-4 col-lg-offset-4">
        ...
        <div id="chat-window" class="col-lg-12">
            @foreach($mesazhe as $mesazhe)
                <b> {{ $mesazhe->sender }}</b>/  <i> {{ $mesazhe->created_at }}</i> <br>{{ $mesazhe->msg }} <br><br>
            @endforeach
        </div>
        <div class="col-lg-12">
    ...
    <script type="text/javascript">
        ...
        function retrieveChatMessages()
        {
            ...
            $.ajax({
                ...
                {
                    //if(data.length>0) {
                        $('#chat-window').append('<br><<b>'+data.sender+ '</b> / ' +data.created_at+'<br><hr>'+data.msg+'<br>');
                    //}
                }
            });
        }
        ...
        function sendMessage() {
            ...
            if (text.length > 0) {
                $.ajax({
                    url: "{{ url('admin/sent/chat/messages') }}",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    method: 'POST',
